function [pos,el, az, dop, range, trop_TSA, vector, pd_omc, pd_vector, pd_x, delta_r] = leastSquarePos_TSA2(pos_last,satpos,obs,settings, spoof_value2, spoof_order, spoof_time, currMeasNr)
...
el      = az;
Rot_X_10 = zeros(size(satpos, 1), nmbOfSatellites);
range   = zeros(1, nmbOfSatellites);
vector  = zeros(3, nmbOfSatellites);
trop_TSA = zeros(1, nmbOfSatellites);
...
for iter = 1:nmbOfIterations
    ...
    if  currMeasNr >= spoof_time
        delta_t = (spoof_value2) * 1/factorial(spoof_order)* (settings.navSolPeriod / 1000 * (currMeasNr-spoof_time)).^spoof_order;
        H = pinv(A'*A)*A';%(A'*A)\A';
        A_null_space = null( H );
...
% save for TSA
for i_TSA = 1:nmbOfSatellites
    range(i_TSA) = norm(Rot_X_10(:,i_TSA) - pos(1:3), 'fro');
    vector(:,i_TSA) = (Rot_X_10(:,i_TSA) - pos(1:3))/range(i_TSA);
end
