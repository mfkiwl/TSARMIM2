...
for currMeasNr = 1:measNrSum
    ...
    navSolutions.satClkCorr(:, currMeasNr) = ...
                                         NaN(settings.numberOfChannels, 1);                                                                  
    navSolutions.timing(:, currMeasNr) = ...
                                         NaN(settings.numberOfChannels, 1);

    %TSA
    navSolutions.range(:, currMeasNr) = NaN(settings.numberOfChannels, 1);
    navSolutions.trop_TSA(:, currMeasNr) = NaN(settings.numberOfChannels, 1);
    navSolutions.doppler (:, currMeasNr) = NaN(settings.numberOfChannels, 1);
    navSolutions.velrange(:, currMeasNr) = NaN(settings.numberOfChannels, 1);
    navSolutions.accrange(:, currMeasNr) = NaN(settings.numberOfChannels, 1);
    navSolutions.satClkCorrRat(activeChnList, currMeasNr) = ...
                                         NaN(settings.numberOfChannels, 1);
    navSolutions.satClkAcc(activeChnList, currMeasNr) = ...
                                         NaN(settings.numberOfChannels, 1);
    navSolutions.pdvector{currMeasNr} = NaN;
    navSolutions.pdomc(:, currMeasNr) = NaN(settings.numberOfChannels, 1);
    navSolutions.vel_H{currMeasNr} = NaN;
    navSolutions.vel_f(:, currMeasNr) =NaN(settings.numberOfChannels, 1);
...
%% Find pseudoranges ======================================================
    ...
    [navSolutions.rawP(:, currMeasNr),transmitTime,localTime, index]=  ...
...
%% Find satellites positions and clocks corrections =======================
    ...
    [satPositions, satClkCorr, satVolocity, satClkCorrRat, satAcc, satClkAcc] = satpos(transmitTime(activeChnList), ...
    ...
    % TSA
    navSolutions.satpos(1, currMeasNr) = {satPositions};
    navSolutions.satClkCorrRat(activeChnList, currMeasNr) = satClkCorrRat;
    navSolutions.satClkAcc(activeChnList, currMeasNr) = satClkAcc;
%% Find receiver position =================================================
    ...
    if size(activeChnList, 2) > 3
        ...
        %        navSolutions.DOP(:, currMeasNr), navSolutions.range(activeChnList, currMeasNr), navSolutions.trop_TSA(activeChnList, currMeasNr), vector, pd_omc, pd_vector, pd_x] =...
        %                leastSquarePos_TSA(pos_last,satPositions, clkCorrRawP, settings);
        % [navSolutions.velrange(activeChnList, currMeasNr),  velxyzdf, vel_H, vel_f] =...
        %                leastSquareVel_TSA(vector, satVolocity, satClkCorrRat, navSolutions.doppler(:, currMeasNr), xyzdt, settings);
        ...
        % TSA Strategy 2
        spoof_value = 0.1 * 10^(-9) * settings.c;
        spoof_value1 = 1 * 10^(-9) * settings.c;
        spoof_value2 = 0.1 * 10^(-9) * settings.c;
        spoof_order = 3;
        spoof_time = 60 / settings.navSolPeriod *1000;
        start_num = 60 / settings.navSolPeriod *1000;
        end_num = 120 / settings.navSolPeriod *1000;
        [xyzdt,navSolutions.el(activeChnList, currMeasNr), ...
               navSolutions.az(activeChnList, currMeasNr), ...
               navSolutions.DOP(:, currMeasNr), navSolutions.range(activeChnList, currMeasNr), navSolutions.trop_TSA(activeChnList, currMeasNr), vector, pd_omc, pd_vector, pd_x, delta_r] =...
                       leastSquarePos_TSA2(pos_last,satPositions, clkCorrRawP, settings, spoof_value2, spoof_order, spoof_time, currMeasNr);
        [navSolutions.velrange(activeChnList, currMeasNr),  velxyzdf, vel_H, vel_f, delta_rv] =...
                       leastSquareVel_TSA2(vector, satVolocity, satClkCorrRat, navSolutions.doppler(:, currMeasNr), xyzdt, settings, spoof_value2, spoof_order, spoof_time, currMeasNr);
        navSolutions.doppler(:, currMeasNr) = navSolutions.doppler(:, currMeasNr) - delta_rv;
        navSolutions.rawP(activeChnList, currMeasNr) = navSolutions.rawP(activeChnList, currMeasNr) + delta_r;
       
        %


        navSolutions.pdvector{currMeasNr} = pd_vector;
        navSolutions.pdomc(activeChnList, currMeasNr) = pd_omc;
        navSolutions.pd_x(:, currMeasNr) = pd_x;

        

        navSolutions.vel_H{currMeasNr} = vel_H;
        navSolutions.vel_f(activeChnList, currMeasNr) = vel_f;
        for sat_num = 1 : size(activeChnList, 2)
            navSolutions.accrange(sat_num, currMeasNr) = (satAcc(:,sat_num))'*vector(:,sat_num);
        end
        %=== Save results ===========================================================
        ...
        navSolutions.vx(currMeasNr) = velxyzdf(1);
        navSolutions.vy(currMeasNr) = velxyzdf(2);
        navSolutions.vz(currMeasNr) = velxyzdf(3);
        navSolutions.df(currMeasNr) = velxyzdf(4);
...
%% Coordinate conversion ==================================================
        ...
        navSolutions.el(activeChnList, currMeasNr) = ...
                                             NaN(1, length(activeChnList));
        navSolutions.timing(activeChnList,currMeasNr)=...
                                             NaN(1, length(activeChnList));
       
       %TSA
       navSolutions.range(activeChnList, currMeasNr) = ...
                                             NaN(1, length(activeChnList));
       navSolutions.trop_TSA(activeChnList, currMeasNr) = ...
                                             NaN(1, length(activeChnList));
       navSolutions.doppler(activeChnList, currMeasNr) = ...
                                             NaN(1, length(activeChnList));
       navSolutions.vx(currMeasNr) = NaN;
       navSolutions.vy(currMeasNr) = NaN;
       navSolutions.vz(currMeasNr) = NaN;
       navSolutions.df(currMeasNr) = NaN;
       navSolutions.velrange(activeChnList, currMeasNr) = ...
                                             NaN(1, length(activeChnList));
       navSolutions.accrange(activeChnList, currMeasNr) = ...
                                             NaN(1, length(activeChnList));
       navSolutions.pdvector{currMeasNr} = NaN;
       navSolutions.pdomc(:, currMeasNr) = NaN(settings.numberOfChannels, 1);
       navSolutions.vel_H{currMeasNr} = NaN;
